...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions
{
    ...
    self.organization = @"3alh1w";
    self.deviceID = @"iot-starter-android";
    self.authToken = @"iot-starter-android";
    ...
    PlayAroundFile *myClass = [PlayAroundFile new]; // KAD
    
    [myClass initSuperApp];
   // [myClass makeGetCallWithUrlInput:@"https://jsonplaceholder.typicode.com/todos/1" callBackXX: PlayAroundFile.processJSONResult];// KAD
   
    //[myClass makePostCallWithUrl:@"https://jsonplaceholder.typicode.com/todos"];
...
- (void)startMotionManager
{
    ...
    self.locationRequestID = [locMgr subscribeToLocationUpdatesWithBlock:^(CLLocation *currentLocation, INTULocationAccuracy achievedAccuracy, INTULocationStatus status) {
        ...
        if (status == INTULocationStatusSuccess) {
            ...
            [myClass processLocationChanges];
            
                NSLog(@"KEVIN %f", self.locationManagerNew.location.coordinate.latitude);
        }
                              }];
    if (false) //!self.locationManagerNew)
    {
        self.locationManagerNew = [[CLLocationManager alloc] init];
        self.locationManagerNew.delegate = self;

        self.locationManagerNew.desiredAccuracy = kCLLocationAccuracyBestForNavigation;
        //self.locationManager.startMonitoringSignificantLocationChanges;
        //self.locationManager.pausesLocationUpdatesAutomatically = false;
        //self.locationManager.allowsBackgroundLocationUpdates = true;
        self.locationManagerNew.distanceFilter = 1;
        
        if (IS_OS_8_OR_LATER) {
            [self.locationManagerNew requestWhenInUseAuthorization];
            
            [self.locationManagerNew requestAlwaysAuthorization];
            NSLog(@"IN HERE Location");
        }
        [self.locationManagerNew startUpdatingLocation];
    }
...
/** Disable the accelerometer and detector.
 */
- (void)stopMotionManager
{
    ...
    [self.locationManagerNew stopUpdatingLocation];
...
- (void)playSpeech:(NSString *)textValue
{
    PlayAroundFile *myClass = [PlayAroundFile new]; // KAD
    
    [myClass speakWordsWithWords:textValue];
}
- (void)displayViewController:(NSString *)textValue
{
...
- (void)locationManager:(CLLocationManager *)manager didUpdateToLocation:(CLLocation *)newLocation fromLocation:(CLLocation *)oldLocation {
    ...
    NSLog(@"Kevin N%@", newLocation);
    NSLog(@"Kevin %@", self.locationManagerNew.location);
    NSLog(@"Kevin O%@", oldLocation);
...
{
    NSLog(@"location Manager failed %@ %@", error.localizedDescription, error.localizedFailureReason);
}
/*
